...
export const TitleValidatedTextField = ({
    name,
    validator,
    onChange,
    value,
    ...
    const handleChange = (e) => {
        ...
        if (errorMessage === 'SymbolError') {
            ...
            setError(
                `Назва публікації повинна мати не менше ${minChars}-х символів`
            )
            setValue(newValue)
        } else if (errorMessage === 'MaxLengthError') {
            setError(
                `Назва публікації повинна мати не більше ${maxChars}-ти символів`
            )
        } else {
...
export const titleValidator = (value, minChars, maxChars) => {
