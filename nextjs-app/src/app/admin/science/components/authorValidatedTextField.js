...
export const AuthorValidatedTextField = ({
    name,
    validator,
    onChange,
    value,
    ...
    const handleChange = (e) => {
        ...
        if (errorMessage === 'SymbolError') {
            ...
            setError(`Ім'я автора повинно бути не менше ${minChars}-х символів`)
            setValue(newValue)
        } else if (errorMessage === 'MaxLengthError') {
            setError(
                `Ім'я автора повинно бути не більше ${maxChars}-ти символів`
            )
        } else {
...
export const authorValidator = (value, minChars, maxChars) => {
