...
export const CategoryValidatedTextField = ({
    name,
    validator,
    onChange,
    value,
    ...
    const handleChange = (e) => {
        ...
        if (errorMessage === 'SymbolError') {
            ...
            setError(
                `Назва категорії повинна мати не менше ${minChars}-х символів`
            )
            setValue(newValue)
        } else if (errorMessage === 'MaxLengthError') {
            setError(
                `Назва категорії повинна мати не більше ${maxChars}-ти символів`
            )
        } else {
...
export const categoryValidator = (value, minChars, maxChars) => {
